...
public class AuthController {
    ...
    private ResponseEntity<?> handleSignup(@Valid @RequestBody SignupUserDto signupUserDto, BindingResult bindingResult) throws Exception {
        ...
        if (userRepository.findByEmailAndProvider(signupUserDto.getEmail(), Provider.LOCAL) != null) {
    ...
    @PostMapping("/login")
    private ResponseEntity<?> handleSignin(@Valid @RequestBody SigninUserDto signinUserDto, BindingResult bindingResult) throws Exception {
        if (bindingResult.hasErrors()) {
            throw new BindException(bindingResult);
        }
        UserModel userModel = userRepository.findByEmailAndProvider(signinUserDto.getEmail(), Provider.LOCAL);
        if (userModel == null) {
    ...
    public ResponseEntity<?> handleRefresh() throws Exception {
        ...
        if (authentication != null) {
            ...
            String access = jwtService.generateAccessToken(model);
